    ...
    skeleton: (
      <Box sx={{ padding: '40px 20px', maxWidth: '1200px', margin: '0 auto' }}>
        <Skeleton
          ...
          width="30%"
          height={40}
          sx={{ margin: '0 auto 20px', bgcolor: 'rgba(30, 58, 138, 0.1)' }}
        />
        <Skeleton
          variant="text"
          width="60%"
        ...
        <Box sx={{ display: 'flex', gap: 2, justifyContent: 'center', flexWrap: 'wrap' }}>
          {[1, 2, 3, 4].map((i) => (
            <Skeleton
              ...
              width={250}
              height={180}
              sx={{ bgcolor: 'rgba(30, 58, 138, 0.05)' }}
            />
          ))}
        </Box>
      </Box>
...
const BackToTopButton = memo(() => {
  ...
  // Only render on desktop (>=1024px); hidden on mobile and tablet so it
  // doesn't crowd the StickyMobileCTA bar or the floating contact stack.
    ...
    <AnimatePresence>
      {isVisible && isDesktop && (
        <motion.button
          ...
          whileHover={{ scale: 1.1, backgroundColor: '#152659' }}
          whileTap={{ scale: 0.9 }}
          onClick={scrollToTop}
          style={{
            position: 'fixed',
